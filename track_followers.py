...
if len(sys.argv) == 2:
    ...
    new_list = subprocess.check_output(
        ["twitter-follow", "-o", "-r", "-i", my_handle]).decode("utf-8").strip().split('\n')
...
if not os.path.exists(followers_path):
    sys.stderr.write(
        "WARNING: %s not found; generating new list from scratch.\n" % followers_path)
else:
    ...
    gained = []
    renamed = []
    for userid in old_userid_set - new_userid_set:2
    ...
    if lost or gained or renamed:
        log_lines.append("-------- %s:" %
                         time.strftime("%Y-%m-%d %H:%M:%S %Z", timestamp))
        if lost:
        ...
        if renamed:
            ...
            for userid, old_name, new_name in sorted(renamed):
                log_lines.append("\t%10d %s -> %s" %
                                 (userid, old_name, new_name))
        log_lines.append("")
